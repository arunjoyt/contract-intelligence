...
- `evaluation/evaluate.py` — `OpenAI(api_key=os.environ["OPENAI_API_KEY"])` in `evaluate()`
...
| `evaluation/evaluate.py` | `_run_question()` `openai_client.chat.completions.create(...)`, temp 0.0 — mirrors `query_pipeline._generate()` (shares its prompt / knobs via `pipeline/constants.py`) | `response.choices[0].message.content` |
