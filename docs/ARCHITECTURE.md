...
OpenAI calls). It also records that self-hosted Langfuse 2.x prices `gpt-4o` at
its stale mid-2024 launch rate (~1.9× current list — #137), so `totalCost` in the
UI reads high; `results.json`'s `costs` block (#130) uses current rates.
