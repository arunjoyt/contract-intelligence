...
`docs/BENCHMARKS.md` aggregates these traces into a reproducible per-stage latency
and per-query/per-ingest cost baseline — regenerated by
`scripts/benchmark_from_langfuse.py` (read-only against the Langfuse API, no
OpenAI calls). Self-hosted Langfuse 2.x's bundled table prices `gpt-4o` at its
stale mid-2024 launch rate (~1.9× current list — #137); `scripts/langfuse_fix_model_prices.py`
adds a project-level override at the current rate (run per deployment — see
`docs/CLIENT_DEPLOYMENT_RUNBOOK.md` Phase 3), after which the UI `totalCost` and
`results.json`'s `costs` block (#130) agree.
