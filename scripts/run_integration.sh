...
#   integration_<timestamp>.html   -- self-contained, browsable pass/fail report
set -uo pipefail
...
HTML_FILE="$RESULTS_DIR/integration_${TIMESTAMP}.html"
...
RUN_INTEGRATION=1 "$REPO_ROOT/.venv/bin/pytest" "$REPO_ROOT/tests/test_integration.py" \
    -v --html="$HTML_FILE" --self-contained-html "$@" 2>&1 | tee "$LOG_FILE"
status="${PIPESTATUS[0]}"
